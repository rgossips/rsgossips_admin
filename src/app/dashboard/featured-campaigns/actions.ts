...
export async function addFeaturedCampaign(campaignId: string, position?: number): Promise<{ error?: string; ok?: boolean }> {
  ...
  // pick lands at the bottom and admin can reorder from there.
  let pos = position ?? 0;
  if (position == null) {
  ...
  const { error } = await admin
    .from("featured_campaigns")
    .insert({ campaign_id: campaignId, position: pos, is_active: true });
  if (error) return { error: error.message };
...
export async function searchCampaignsForFeature(query: string) {
  ...
  // De-dupe and decorate with brand display info for the dropdown
  ...
  // For campaigns we found by title, do a second lookup to get brand info
  ...
  // Hide already-featured campaigns so the picker can't double-add
  const { data: already } = await admin.from("featured_campaigns").select("campaign_id");
