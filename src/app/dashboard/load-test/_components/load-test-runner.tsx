...
export function LoadTestRunner({ scenarios }: { scenarios: { id: string; label: string }[] }) {
  const t = useTranslations("DashboardLoadTestComponentsLoadTestRunner");
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-5 space-y-4">
        <p className="text-[11px] font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{t("scenarios")}</p>
        ...
        <div className="grid grid-cols-2 gap-4 max-w-md">
          <div>
            <label className="block text-[13px] font-medium text-gray-700 dark:text-gray-300 mb-1.5">
              {t("virtualUsers")} <span className="text-gray-400">{t("maxHint", { max: 20 })}</span>
            </label>
            <input type="number" min={1} max={20} value={vus} onChange={(e) => setVus(Number(e.target.value))} className={inputClass} />
          </div>
          <div>
            <label className="block text-[13px] font-medium text-gray-700 dark:text-gray-300 mb-1.5">
              {t("iterationsPerUser")} <span className="text-gray-400">{t("maxHint", { max: 20 })}</span>
            </label>
        ...
        <div className="flex items-center gap-4">
          ...
          >
            ...
            {running ? t("running") : t("runLoadTest")}
          </button>
          <p className="text-[12px] text-gray-400">
            {t("totalRequestsSummary", {
      ...
      {results && (
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
            <p className="text-[11px] font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{t("results")}</p>
            {ranAt && (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-[11px] uppercase tracking-wider text-gray-400 border-b border-gray-100 dark:border-gray-800">
                  <th className="px-5 py-3">{t("colScenario")}</th>
                  <th className="px-3 py-3 text-right">{t("colOkTotal")}</th>
                  <th className="px-3 py-3 text-right">{t("colErrors")}</th>
                  <th className="px-3 py-3 text-right">{t("colReqPerSec")}</th>
                  <th className="px-3 py-3 text-right">{t("colP50")}</th>
                  <th className="px-3 py-3 text-right">{t("colP95")}</th>
                  <th className="px-3 py-3 text-right">{t("colMax")}</th>
                  <th className="px-5 py-3 text-right">{t("colAvgKb")}</th>
                </tr>
              </thead>
