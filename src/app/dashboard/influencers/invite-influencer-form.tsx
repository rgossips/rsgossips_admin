      ...
      <div className={`${inputClass} flex flex-wrap items-center gap-1.5 min-h-[42px] !py-1.5 !px-2.5`}>
        {tags.map((t) => (
          <span key={t} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-[11px] font-semibold">
            {t}
            <button type="button" onClick={() => onChange(tags.filter((x) => x !== t))} className="hover:text-red-500 cursor-pointer">
              &times;
            </button>
          </span>
        ))}
...
export function InviteInfluencerForm() {
  ...
  const handleSubmit = async (formData: FormData) => {
    setError("");
    setSuccess("");
    // Validate required fields BEFORE the photo upload — otherwise a submit
    // that will fail server-side ("City is required") still uploads and
    ...
    try {
      ...
      // Cities join into one comma-separated "city" field (guaranteed
      // non-empty by the validation above). Downstream matcher splits it.
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl overflow-hidden">
          ...
          <form action={handleSubmit} className="p-6 space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <MultiSelectChips label={t("contentLanguage")} options={INDIAN_LANGUAGES} selected={selectedLanguages} onChange={setSelectedLanguages} />
            </div>
