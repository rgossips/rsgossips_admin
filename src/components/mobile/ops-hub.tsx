...
const CARDS: Card[] = [
  {
    // First: an unapproved campaign is invisible to every creator until an
    // admin acts, so it is the most time-critical queue on the list.
    href: "/dashboard/campaigns?status=under_review",
    key: "campaignsUnderReview",
    labelKey: "campaignReview",
    tone: "text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-900/20",
    icon: (
  ...
  {
    // A pending brand can't publish anything until verified.
    href: "/dashboard/brands?verification=pending",
    key: "brandVerifications",
    labelKey: "brandVerifications",
    tone: "text-sky-600 dark:text-sky-400 bg-sky-50 dark:bg-sky-900/20",
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
